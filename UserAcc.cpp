...
{
	/*
		When a user creates an account they can't have a person with a similar username to someone in the database or else it can't pick the right acccount same with password
		So will check is said user input name is available, if so will take the name if not will ask user to input a new user till they find one same with password
	*/
